jQuery(function($) {
  var view_premium = "/app/views/premium.html";
  var view_simple = "/app/views/simple.html";
  if(document.location.pathname === view_premium) {
	...
	dl.get_orders().done(function(data, status, xhr) {
	  if(data && data[0]) {
	    var d = data[0].csv_order;
	    if(d && d.file_generated_at !== null) {
	      $('#file_generated_at').text(
		$.format.date(d.file_generated_at, "yyyy/MM/dd HH:mm:ss"))
